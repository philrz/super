...
# SuperDB is the only known tool set that outputs data in BSUP formats. Sample
# BSUP data from super is stored in the https://github.com/brimdata/zed-sample-data
# repo. Therefore, if a change in SuperDB causes the BSUP output format to change,
# we'll want to know about it ASAP, since if it's a bug we'll want to fix it
# in SuperDB, and if it's an intentional enhancement we'll want to update the BSUP
...
if (( BSUP_SUCCESS == 0 && BSUP_UNCOMPRESSED_SUCCESS == 0 && SUP_SUCCESS == 0)); then
  ...
  echo "repo with new output files and MD5 hashes to make this test pass, or open a SuperDB"
